...
Use DatabaseTransactions in every test that may write to the database. Individual tests and their setup must not refresh, truncate, or migrate the
database. This restriction does not apply to intentional reset workflows run outside tests, such as `composer fresh` or `composer test-fresh`.
