<?php

namespace App\Domain\Life\Action;

use App\Domain\CacheKey;
use App\Domain\Life\Models\Trip;
use App\Domain\Life\Scope\TripPublishedScope;
use Illuminate\Cache\Repository;

class GetTripsPublishedByCountryAction
{
    public function __construct(private Repository $cache) {}

    public function execute(int|null $countryId = null): array
    {
        $key = CacheKey::TripsPublishedByCountry;

        $ids = $this->cache->remember($key, $key->ttl(), static function () {
            return Trip::query()
                ->tap(new TripPublishedScope)
                ->with('city:id,country_id')
                ->get(['id', 'city_id'])
                ->mapToGroups(static fn (Trip $trip) => [$trip->city->country_id => $trip->id])
                ->toArray();
        });

        if ($countryId) {
            return $ids[$countryId] ?? [];
        }

        return $ids;
    }
}
