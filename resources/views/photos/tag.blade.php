...
@section('content')
<h3>#{{ $tag->title }} <small class="tw-text-base tw-text-gray-600">{{ sizeof($photos) }}</small></h3>
<div class="tw-flex tw-flex-wrap tw-mobile-wide">
  @foreach ($photos as $photo)
    <div class="tw-w-full sm:tw-w-1/2 lg:tw-w-1/3 tw-mx-auto sm:tw-mx-0">
      <a class="tw-block tw-relative tw-w-full tw-pb-3/4" href="{{ path("$self@show", [$photo, $tag->getForeignKey() => $tag]) }}">
