...
<div class="grid gap-8">
  <x-terminal>
    <div class="grid gap-6">
      ...
      <div>
        <div class="text-muted dark:text-white"># После больших обновлений вроде 10.0 => 11.0 добавляется две команды</div>
        <div class="prepend-dollar">synth upgrade-system</div>
        <div class="prepend-dollar">freebsd-update install</div>
      </div>

      <div>
        <div class="text-muted dark:text-white"># Обновление портов</div>
  ...
  <x-terminal>
    <div class="text-muted dark:text-white"># Чистка портов</div>
    <div class="prepend-dollar"><a class="link" href="https://www.freebsd.org/cgi/man.cgi?query=pkg">pkg</a> clean</div>
  </x-terminal>

  <x-terminal>
    <div class="text-muted dark:text-white"># Исходники ядра</div>
  ...
  <x-terminal>
    <div class="text-muted dark:text-white"># Генерация зашифрованного пароля</div>
    <div class="prepend-dollar"><a class="link" href="https://www.freebsd.org/cgi/man.cgi?query=openssl">openssl</a> passwd -1</div>
  </x-terminal>

  <x-terminal>
    <div class="text-muted dark:text-white"># Билд ядра</div>
  ...
  <x-terminal>
    <div class="text-muted dark:text-white"># Первоначальная подготовка портов</div>
  ...
  <x-terminal>
    <div class="grid gap-6">
      ...
      <div>
        <div class="text-muted dark:text-white"># Состояние виртуальной памяти</div>
  ...
  <x-terminal>
    <div class="grid gap-6">
      <div>
        <div class="text-muted dark:text-white"># Создание файла подкачки на 2048 МБ</div>
      ...
      <div>
        <div class="text-muted dark:text-white"># Добавление записи в /etc/fstab для использования при следующей загрузке системы</div>
        <div>md99 none swap sw,file=/usr/swap0,late 0 0</div>
      </div>

      <div>
        <div class="text-muted dark:text-white"># Активация файла подкачки без перезагрузки</div>
  ...
  <x-terminal>
    <div class="text-muted dark:text-white"># Обновление jenkins без обновления openjdk</div>
    <div class="prepend-dollar">portmaster -bdg -x openjdk8 devel/jenkins-lts</div>
  </x-terminal>

  <x-terminal>
    <div class="text-muted dark:text-white"># Смена оболочки по умолчанию</div>
