<?php /** @var \App\Domain\Wanikani\Livewire\KanjiList $this */ ?>
  ...
  @if ($this->kanjis->count())
    ...
    <div class="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-px text-center text-white">
      @foreach ($this->kanjis as $kanji)
        <div class="group rounded-sm {{ auth()->id() && isset($this->burned[$kanji->id]) ? 'bg-burned' : 'bg-kanji' }} {{ auth()->id() && !$this->showBurned && isset($this->burned[$kanji->id]) ? 'hidden' : '' }}">
          <a
