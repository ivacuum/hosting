...
<div class="flex items-center justify-between mt-4">
  <div>
    @if ($level > 1)
      <a href="{{ path(App\Http\Controllers\WanikaniLevelController::class, $level - 1) }}" id="prev_page">
        @svg (chevron-left)
  ...
  <div>
    @if ($level < 60)
      <a href="{{ path(App\Http\Controllers\WanikaniLevelController::class, $level + 1) }}" id="next_page">
        @lang('Уровень :level', ['level' => $level + 1])
        @svg (chevron-right)
      </a>
    @endif
