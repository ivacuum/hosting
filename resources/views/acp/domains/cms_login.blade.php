<form class="d-inline" method="post" action="{{ $model->alias_id ? $model->alias->cms_url : $model->cms_url }}" target="_blank">
  @if ($model->cms_type == 'korden.cms' || $model->cms_type == 'simpla')
    <input type="hidden" name="login" value="{{ $model->cms_user }}">
    <input type="hidden" name="password" value="{{ $model->cms_pass }}">
    <button type="submit" name="loginsubmit" class="{{ $cms_button_class ?? '' }}">
      @svg (sign-in)
    </button>
  @elseif ($model->alias_id && ($model->alias->cms_type == 'korden.cms' || $model->alias->cms_type == 'simpla'))
  ...
  @elseif ($model->cms_type == 'integrium')
    ...
    <button type="submit" class="{{ $cms_button_class ?? '' }}">
      @svg (sign-in)
    </button>
  ...
  @elseif ($model->cms_type == 'modx')
    ...
    <button type="submit" name="login" class="{{ $cms_button_class ?? '' }}" value="1">
      @svg (sign-in)
    </button>
