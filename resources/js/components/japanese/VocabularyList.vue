      ...
      <template v-for="(collection, lvl) in filteredElements">
        ...
        <div class="text-xl text-center md:text-left vocab-grid">
          <template v-for="row in collection">
            <div>
              <router-link
                class="bg-vocab inline-block text-4xl leading-none ja-shadow-light px-2 py-1 rounded whitespace-no-wrap text-white hover:text-grey-200"
            ...
            <div
              ...
              :id="`kana-${row.id}`"
            >
              <div class="leading-none whitespace-no-wrap" v-for="kana in row.kana.split(', ')">
                【{{ kana }}】
              </div>
            </div>
            <div
            ...
            >
              <svg class="svg-icon svg-flame" viewBox="0 0 12 16"><path d="M5.05.31c.81 2.17.41 3.38-.52 4.31C3.55 5.67 1.98 6.45.9 7.98c-1.45 2.05-1.7 6.53 3.53 7.7-2.2-1.16-2.67-4.52-.3-6.61-.61 2.03.53 3.33 1.94 2.86 1.39-.47 2.3.53 2.27 1.67-.02.78-.31 1.44-1.13 1.81 3.42-.59 4.78-3.42 4.78-5.56 0-2.84-2.53-3.22-1.25-5.61-1.52.13-2.03 1.13-1.89 2.75.09 1.08-1.02 1.8-1.86 1.33-.67-.41-.66-1.19-.06-1.78C8.18 5.31 8.68 2.45 5.05.32L5.03.3l.02.01z"/></svg>
            </a>
          </template>
        </div>
      </template>
...
export default {
  ...
  computed: {
    ...
    showGroupActionButtons() {
      return this.level === undefined && !this.kanji
    },
  ...
  methods: {
    ...
    loadData() {
      axios
        .get(`${locale}/japanese/wanikani/vocabulary`, {
          params: {
            kanji: this.kanji,
            level: this.level,
          }
